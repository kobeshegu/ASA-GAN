...
class ChannelAttention_D(nn.Module):
    def __init__(self, in_planes, ratio=16):
        super(ChannelAttention_D, self).__init__()
        # print("-------")
        # print(in_planes)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)
        ...
        self.fc = nn.Sequential(nn.Conv2d(in_planes // 4, in_planes // 64, 1, bias=False),
                                nn.BatchNorm2d(in_planes // 64 ),
                                nn.ReLU(),
                                nn.Conv2d(in_planes // 64, in_planes // 4, 1, bias=False))
        self.sigmoid = nn.Sigmoid()
...
class Discriminator(nn.Module):
    def __init__(self, ndf=64, nc=3, im_size=512):
        ...
        self.catten = ChannelAttention_D(im_size)
        self.satten = SpatialAttention()
    ...
    def forward(self, imgs, data_rf):
       ...
       # print(feat_8.size())
#        feat_ca = self.catten(feat_8) * feat_8
  #      feat_sa = self.satten(feat_ca) * feat_ca
        feat_16 = self.down_16(feat_8)
       # print(feat_16.size())
       # torch.size([4,32,128,128])
       # feat_16 = self.se_2_16(feat_2, feat_16)
        feat_ca_16 = self.catten(feat_16) * feat_16
        feat_sa_16 = self.satten(feat_ca_16) * feat_ca_16
        feat_16 = self.down_16(feat_8)
       ...
       # print(feat_last.size())
        ...
        if data_rf == "real":
            ...
            return feat_out , [rec_img_big, rec_img_small, rec_img_part], part, rf_final

        # return torch.cat([rf_0, rf_1])
        return feat_out , rf_final
